...
export default function SplashScreen({ onFinish }: { onFinish: () => void }) {
  ...
  useEffect(() => {
    ...
    // Total duration is roughly 3000ms
    // We start the exit animation a bit earlier to make it smooth
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center overflow-hidden transition-opacity duration-1000 ease-in-out ${isExiting ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      {/* Background Effects */}
